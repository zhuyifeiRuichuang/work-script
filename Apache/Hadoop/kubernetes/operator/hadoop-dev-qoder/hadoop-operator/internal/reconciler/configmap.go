...
// HadoopClusterReconciler holds the reconciler state for all components
...
// computeConfigHash computes a hash of the ConfigMap data for change detection
func (r *HadoopClusterReconciler) computeConfigHash(data map[string]string) string {
...
// getConfigHash retrieves the current ConfigMap and computes its hash
func (r *HadoopClusterReconciler) getConfigHash(ctx context.Context, cluster *hadoopv1.HadoopCluster) string {
	cm := &corev1.ConfigMap{}
	cmKey := client.ObjectKey{Name: cluster.Name + "-config", Namespace: cluster.Namespace}
	if err := r.Get(ctx, cmKey, cm); err != nil {
		return ""
	}
	return r.computeConfigHash(cm.Data)
}

// podTemplateAnnotations returns annotations for StatefulSet pod templates
// including the config hash for triggering rolling updates on config changes
func (r *HadoopClusterReconciler) podTemplateAnnotations(ctx context.Context, cluster *hadoopv1.HadoopCluster) map[string]string {
	return map[string]string{
		"hadoop.apache.org/config-hash": r.getConfigHash(ctx, cluster),
	}
}
...
func (r *HadoopClusterReconciler) generateHadoopConfig(cluster *hadoopv1.HadoopCluster) map[string]string {
	...
	// hadoop-env.sh - JVM and environment configuration
	config["hadoop-env.sh"] = r.generateHadoopEnv(cluster)
...
// generateHadoopEnv generates hadoop-env.sh with JVM heap configuration
func (r *HadoopClusterReconciler) generateHadoopEnv(cluster *hadoopv1.HadoopCluster) string {
	var sb strings.Builder
	sb.WriteString("#!/bin/bash\n")
	sb.WriteString("# Auto-generated by Hadoop Operator - Do not edit manually\n\n")

	// NameNode heap size
	nnHeap := r.computeHeapSize(cluster.Spec.HDFS.NameNode.Resources, "1024")
	sb.WriteString(fmt.Sprintf("export HADOOP_NAMENODE_HEAPSIZE=%s\n", nnHeap))

	// DataNode heap size
	dnHeap := r.computeHeapSize(cluster.Spec.HDFS.DataNode.Resources, "1024")
	sb.WriteString(fmt.Sprintf("export HADOOP_DATANODE_HEAPSIZE=%s\n", dnHeap))

	// ResourceManager heap size
	rmHeap := r.computeHeapSize(cluster.Spec.YARN.ResourceManager.Resources, "1024")
	sb.WriteString(fmt.Sprintf("export YARN_RESOURCEMANAGER_HEAPSIZE=%s\n", rmHeap))

	// NodeManager heap size
	nmHeap := r.computeHeapSize(cluster.Spec.YARN.NodeManager.Resources, "1024")
	sb.WriteString(fmt.Sprintf("export YARN_NODEMANAGER_HEAPSIZE=%s\n", nmHeap))

	sb.WriteString("\n# General Hadoop settings\n")
	sb.WriteString("export HADOOP_HOME=/opt/hadoop\n")
	sb.WriteString("export HADOOP_CONF_DIR=/opt/hadoop/etc/hadoop\n")
	sb.WriteString("export HADOOP_LOG_DIR=/opt/hadoop/logs\n")
	sb.WriteString("export HADOOP_PID_DIR=/tmp/hadoop-pids\n")
	sb.WriteString("export JAVA_HOME=$(dirname $(dirname $(readlink -f $(which java))))\n")

	return sb.String()
}

// computeHeapSize computes JVM heap size from resource limits (80% of memory limit)
func (r *HadoopClusterReconciler) computeHeapSize(resources corev1.ResourceRequirements, defaultMB string) string {
	if resources.Limits != nil {
		if mem, ok := resources.Limits[corev1.ResourceMemory]; ok {
			memBytes := mem.Value()
			heapMB := int(memBytes / (1024 * 1024) * 80 / 100)
			if heapMB > 0 {
